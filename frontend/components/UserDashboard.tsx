...
export default function UserDashboard({ user, token, logout }: UserDashboardProps) {
  ...
  const fetchTickets = async () => {
    if (!token) {
      console.error("No token available for fetching tickets");
      setIsLoading(false);
      return;
    }
  ...
  const createTicket = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(`Failed to create ticket: ${errorMsg}`);
    }
  };
